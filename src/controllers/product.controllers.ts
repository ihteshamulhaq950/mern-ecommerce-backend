...
interface ICustomMulterRequest extends Request {
    files?: {
...
const getAllProducts = asyncHandler(async (req: Request, res: Response) => {
    ...
    const productAggregate: mongoose.Aggregate<any[]> = Product.aggregate([{ $match: {} }]);
...
const createProduct = asyncHandler(async (req: Request, res: Response) => {
    ...
    let multerRequest = req as ICustomMulterRequest;
    ...
    if (!multerRequest.files?.mainImage || !multerRequest.files.mainImage.length) {
        throw new ApiError(400, "Main image is required");
    }

    const mainImageUrl = getStaticFilePath(
        req,
        multerRequest.files.mainImage[0].filename
    );

    const mainImageLocalPath = getLocalPath(multerRequest.files.mainImage[0].filename);
    ...
    const subImages:
        ...
        = multerRequest.files.subImages && multerRequest.files.subImages.length
            ?
            multerRequest.files.subImages.map((image): { url: string; localPath: string; } => {

                const imageUrl = getStaticFilePath(
                    multerRequest,
                    image.filename
                );
    ...
    const owner = multerRequest.user?._id;
...
const updateProduct = asyncHandler(async (req: Request, res: Response) => {
    ...
    const mainImage: { url: string; localPath: string; } = multerRequest.files?.mainImage?.length
        ? {
            //If user has uploaded new main image then we have to create an object with new url and localpath in the project
            url: getStaticFilePath(
                multerRequest,
                multerRequest.files.mainImage[0].filename
            ),
            localPath: getLocalPath(multerRequest.files.mainImage[0].filename),
        }
        :
        product.mainImage; // If there is no new main image uploded we will stay with the old image of the product
    ...
    let subImages:
        ...
        = multerRequest.files?.subImages && multerRequest.files.subImages.length
            ?
            multerRequest.files.subImages.map((image) => {
    ...
    if (totalSubImages > MAXIMUM_SUB_IMAGE_COUNT) {
        ...
        if (product.mainImage.url !== mainImage.url) {
            // If user has uploaded new main image remove the newly uploaded main image as there is no updation happening
        ...
        throw new ApiError(
            400,
            "Maximum " + MAXIMUM_SUB_IMAGE_COUNT + " sub images are allowed for a product. There are already " + existedSubImages + " sub images attached to the product."
        );
...
const removeProductSubImage = asyncHandler(async (req: Request, res: Response) => {
    ...
    const updatedProduct = await Product.findByIdAndUpdate(productId,
        {
            $pull: {
                // pull an item from subImages with _id equals to the subImageId 
                subImages: {
    ...
    //retrive the file object which is being removed
